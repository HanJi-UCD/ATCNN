%% generate training data for 9 LiFi APs case
% Adaptive UE number with mirror operation  
clear;
clc;
%% paramaters setting
load env_9LiFi.mat env
env.P_mod = 3;
k = 1; % shape parameter in gamma distribution of R
Rb = 200; % scale parameter in gamma distribution of R
batch_size = 256; % batch size
batch_num = 1002; % batch number
AP_num = 10; % 9 LiFi and 1 WiFi
UE_num_list = 40;
X_length = 9; % room size
...
%         X_iu = iterative_LB(env, SSS_X_iu, SNR, R_required); %
    ...
    nor_input_data = input_data; % without normalzzation
